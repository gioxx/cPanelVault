...
- **Concorrenza sicura**: un lock interprocesso per account impedisce a CLI, scheduler e web UI di fare due backup contemporanei dello stesso account cPanel (stesso nome host e stesso username)
...
- Per ogni chiave di lock può girare un solo backup alla volta. Il lock si basa sull'host FTP (ignorando maiuscole, prefisso `ftp.` e punto finale) più `cpanel_username`, quindi vengono serializzate anche voci diverse della configurazione che usano lo stesso nome host per lo stesso account. Voci che raggiungono lo stesso account tramite nomi host *diversi* (es. il dominio del cliente e l'hostname del server del provider) ottengono lock diversi e possono girare in contemporanea, contendendosi lo stesso archivio remoto: usa lo stesso valore di `host`. Un secondo run per una chiave di lock occupata viene saltato con un warning nel log (nessuna notifica, stato invariato; la CLI esce con codice 1). La CLI condivide il lock con la web UI solo se entrambe usano la stessa `LOCK_DIR`, ad esempio `docker compose exec cpanelvault python main.py backup cpanel1`. Nella dashboard la voce effettivamente in esecuzione mostra **Running…**, le altre voci con lo stesso host e lo stesso `cpanel_username` mostrano **Busy**.
